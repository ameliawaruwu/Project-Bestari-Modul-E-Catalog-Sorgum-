...
export async function createProduct(input: CreateProductInput) {
  ...
  const [result] = await dbPool.query(
    `INSERT INTO products (category_id, name, slug, description, price, weight_spec, origin, shipping_info, is_featured, gluten_free, organic, wa_contact, composition, shelf_life, attributes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [category_id, name, slug, description, price, weight_spec, origin, input.shipping_info ?? null, is_featured ? 1 : 0,
...
const ALLOWED_COLUMNS = ['category_id', 'name', 'slug', 'description', 'price', 'weight_spec', 'origin', 'shipping_info', 'is_featured', 'is_active', 'gluten_free', 'organic', 'wa_contact', 'composition', 'shelf_life', 'attributes'];
