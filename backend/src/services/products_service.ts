...
const LIST_SELECT = `
  SELECT p.id, p.name, p.slug, p.description, p.price, p.price_max,
         p.weight_spec, p.origin, p.shipping_info, p.composition, p.shelf_life, p.attributes,
...
export async function createProduct(input: CreateProductInput) {
  const { category_id, name, slug, description, weight_spec, origin, is_featured } = input;
  ...
  const price_max = input.price_max ? Number(input.price_max) : null;
  const [result] = await dbPool.query(
    `INSERT INTO products (category_id, name, slug, description, price, price_max, weight_spec, origin, shipping_info, is_featured, gluten_free, organic, wa_contact, composition, shelf_life, attributes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [category_id, name, slug, description, price, price_max, weight_spec, origin, input.shipping_info ?? null, is_featured ? 1 : 0,
...
const ALLOWED_COLUMNS = ['category_id', 'name', 'slug', 'description', 'price', 'price_max', 'weight_spec', 'origin', 'shipping_info', 'is_featured', 'is_active', 'gluten_free', 'organic', 'wa_contact', 'composition', 'shelf_life', 'attributes'];
