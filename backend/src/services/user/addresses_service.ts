...
export async function createAddress(userId: number, fields: Record<string, any>) {
  const [r] = await dbPool.query(
    `INSERT INTO user_addresses (user_id, label, recipient_name, phone, address_line, city, province, postal_code, is_default)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
export async function updateAddress(addressId: number, userId: number, fields: Record<string, any>) {
  ...
  for (const [k, v] of Object.entries(fields)) {
    if (v !== undefined) { sets.push(`${k} = ?`); vals.push(k === 'is_default' ? (v ? 1 : 0) : v); }
  }
